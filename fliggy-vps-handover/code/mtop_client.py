...
class MtopClient:
    ...
    CURL_BIN = "/usr/bin/curl"  # mac default; on Linux it's usually /usr/bin/curl too

    def __init__(self, cookies: dict[str, str], timeout: int = 15):
        self.cookies = dict(cookies)
        self.timeout = timeout
        # _m_h5_tk 形如 "75e43700f8abc0a74c65078d898c5c18_1787466255406"
        # sign 用前半段（去掉 _<expiry>）
    ...
    def _request(self, url_base: str, data_obj: dict[str, Any], version: str) -> dict:
        ...
        # cookie header (semicolon-separated)
    ...
    def shelf(self, poi_id: str) -> dict:
        """POI 详情页货架响应。返回 mtop 完整 raw（含 data.result.data.shelf.shelves）。"""
        data = {
    ...
    def booktips(self, item_id: str, sku_id: str, poi_id: str) -> dict:
        """预订须知 + 商家信息响应。itemId 级共享，一个 itemId 只 hit 一次。"""
...
def parse_ticket_cells(shelf_raw: dict) -> list[dict]:
    """从 shelf raw 抽所有「门票」cell + sellerId + 价格/库存。详见 docs/04-shelf-ticket-filter.md
